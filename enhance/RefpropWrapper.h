...
public:
    ...
    bool psat_T(double T_K, double& P_kPa, int Qflag = 1);

    // Enthalpy/entropy at (T [K], P [kPa])
    bool hs_from_TP(double T_K, double P_kPa, double& h_Jpkg, double& s_JpkgK);

    // Enthalpy and temperature from (P [kPa], s [J/kg-K])
    bool hT_from_PS(double P_kPa, double s_JpkgK, double& h_Jpkg, double& T_K);
